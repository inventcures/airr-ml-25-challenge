...
# 6. RUN POD 5 (DS7 TRAIN)
# --include ds7 : Only process user 7 datasets
# --only-split train : Only process the training split (~94M sequences)
echo "🚀 Starting POD 5 (DS7 Train)..."
uv run python scripts/generate_embeddings_650m.py --include ds7 --only-split train
